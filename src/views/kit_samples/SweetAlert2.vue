...
function alertImg() {
  Swal.fire({
    imageUrl:
      'https://images.unsplash.com/photo-1724141973274-f3a90b9aa7d8?q=80&w=1887&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D',
    imageWidth: 300,
    // imageHeight: 1200,
...
function alertCustomButton() {
  Swal.fire({
    ...
    // confirm button
    confirmButtonText:
      '<i class="fa fa-thumbs-up"></i>&nbsp;&nbsp;&nbsp;這是自定義的 Confirm button',
...
function alertAutoClose() {
  ...
  console.log(`[alertAutoClose] finish!`);
}
...
function testAjax() {
  ...
  return {
    testOriginalCorrect,
    testOriginalWrong,
    testPromiseChainCorrect,
    testPromiseChainWrong,
    testAsyncPromiseChainCorrect,
    testAsyncPromiseChainWrong,
    testAsyncAwaitCorrect,
    testAsyncAwaitWrong,
    testAjaxLoadingCorrect,
    testAjaxLoadingWrong,
  };
}
...
<template>
  ...
  <h2 class="h2">基本範例</h2>
  <p class="fw-bold">
    注意：SweetAlert 為非同步，開啟之後會繼續執行之後的程式：若要使之同步，可以使用
    <code>async</code>／<code>await</code> 的做法。
  </p>
  ...
  <div class="box d-flex flex-wrap gap-3 mb-52">
    ...
    <button @click="alertOnlyButtonClose" class="btn btn-primary" type="button">
      只能點擊 button 關閉視窗
    </button>
  </div>
  ...
  <div class="box d-flex flex-wrap gap-3 mb-52">
    ...
    <button @click="alertButtonThen" class="btn btn-primary" type="button">
      接收 button 觸發結果
    </button>
    <button @click="alertCustomClassStyle" class="btn btn-primary" type="button">
      自定義套用的 class 樣式
    </button>
  </div>
  ...
  <div class="box d-flex flex-wrap gap-3 mb-52">
    <button @click="alertAutoClose" class="btn btn-primary" type="button">
      時間到自動關閉訊息
    </button>
    <button @click="alertProgressTimer" class="btn btn-primary" type="button">
      有進度條的計時器
    </button>
  ...
  <div class="box d-flex flex-wrap gap-3 mb-52">
    ...
    <button @click="exeByDismissType" class="btn btn-primary" type="button">
      依據關閉方式執行不同動作
    </button>
  </div>
  ...
  <!-- 輸入資料 -->
  <!-- <h2 class="h2">輸入資料</h2>
  <div class="box d-flex flex-wrap gap-3 mb-52"></div> -->
  ...
  <div class="box mb-52">
    ...
    <p>
      使用 async / await 讓 fire 成為同步執行，這時會發現關閉訊息窗的 didClose、didDestroy
      生命週期為異步。<br />
      這邊使用了多個 setTimeout 來約略測量生命週期何時執行，發現它們不只是異步，還而還稍有延遲，約
      250 毫秒左右。
    </p>

    <div class="d-block">
      <button @click="testLifecycleAsync" class="btn btn-primary" type="button">
        willClose 之後的生命週期為異步
      </button>
    </div>
  </div>
  ...
  <div class="box mb-52">
    ...
    <p>
      整個流程是：按下 Enter (keydown 或 keypress) → 關閉訊息窗 → 程式往下繼續執行 → focus 輸入框 →
      鬆開 Enter (keyup) → 觸發訊息窗。
    </p>
    <p>由於程式跑得極快，所以很難避免。</p>
    ...
    <div class="d-block mb-28">
      <label>
        請按下 Enter：
        <input type="text" @keyup.enter="testFocus1" ref="refText1" /> </label
      >&nbsp;&nbsp;
    ...
    <p>
      由之前生命週期的測試，可以知道 didClose、didDestroy 為異步，並且有一些延遲，因此可以把 focus
      寫在這邊，讓 focus 的執行稍慢一點。<br />
    ...
    <div class="d-block mb-28">
      <label>
        請按下 Enter：
        <input type="text" @keyup.enter="testFocus2" ref="refText2" /> </label
      >&nbsp;&nbsp;
    ...
    <div class="d-block mb-28">
      <label>
        請按下 Enter：
        <input type="text" @keyup.enter="testFocus3" ref="refText3" /> </label
      >&nbsp;&nbsp;
    ...
    <p>取得 button 元素，並且使用 Listener 改成在 keyup 時才觸發 button 功能。</p>

    <div class="d-block mb-28">
      <label>
        請按下 Enter：
        <input type="text" @keyup.enter="testFocus4" ref="refText4" /> </label
      >&nbsp;&nbsp;
  ...
  <div class="box mb-52">
    <div class="mb-24">
      ...
      <button @click="alertOriginalLoading" class="btn btn-primary" type="button">
        原始 loading
      </button>
    </div>
    ...
    <div class="mb-24">
      ...
      <button @click="alertMsgAndLoading" class="btn btn-primary" type="button">
        顯示訊息 → 開啟 loading
      </button>
    </div>
    ...
    <div class="mb-24">
      ...
      <button @click="alertLoadingMsg" class="btn btn-primary" type="button">
        開啟 loading → 顯示訊息
      </button>
    </div>
    ...
    <div class="mb-24">
      <p>
        ...
        做法就是不直接調用 loading，而是調用訊息窗，然後在 didOpen (或更早之前的) 生命週期開啟
        loading。<br />
        當要關閉 loading 時，也不是使用 hide loading 的方式，而是直接將訊息窗 close。
      </p>
      <button @click="alertCustomLoading" class="btn btn-primary" type="button">
        客製 loading
      </button>
    </div>
  </div>
  ...
  <div class="box d-flex flex-wrap gap-3 mb-52">
    ...
    <button @click="alertLoadingAndMsg" class="btn btn-primary" type="button">
      訊息 → loading → 訊息
    </button>
  </div>
  ...
  <p>
    因此整個過程就會看到訊息窗被打開又自動關閉的情形，第三個訊息窗甚至有可能還來不及打開就被關閉...
  </p>
  ...
  <div class="box d-flex flex-wrap gap-3 mb-3 mb-52">
    <button @click="testAsyncPromiseChainCorrect" class="btn btn-primary" type="button">
      正確執行
    </button>
    <button @click="testAsyncPromiseChainWrong" class="btn btn-primary" type="button">
      有錯誤
    </button>
    <p>結果：{{ names }}</p>
  </div>
  ...
  <p>
    從上面的各種 loading 測試，可以知道 loading 其實只是一個替代了 Confirm button
    的動態圖示，且其具有依附訊息窗的特性。<br />
    不管是先開 loading 再開訊息窗，還是先開訊息窗再開 loading，loading 都會跟著訊息窗，僅僅替代掉了
    Confirm button 的位置。<br />
    然而，在 AJAX 中所顯示的訊息窗，卻會是一個全新的訊息窗，它的 Confirm button 並沒有被 loading
    替代掉。<br />
    由此可知，在異步線程中，loading 和一般訊息窗可以分開運作。<br />
    但在原線程中，訊息窗的 Confirm button 卻會被 loading 取代。<br />
  </p>
  <p>
    下面做了個測試，在開啟客製 loading 後，再直接連開兩個不同位置的訊息窗，結果第二個訊息窗仍然顯示
    loading 圖示；只有使用了 setTimeout 在 3 秒後顯示的訊息窗，才能呈現自己的 Confirm button。
  </p>
  <p>
  ...
  <div class="box d-flex flex-wrap gap-3 mb-3 mb-52">
    <button @click="alertCustomLoadingMsg" class="btn btn-primary" type="button">
      客製 loading → 顯示訊息
    </button>
  </div>
</template>
